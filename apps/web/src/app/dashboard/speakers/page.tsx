...
export default async function SpeakersPage() {
  ...
  const { data: speakerData } = await supabase
    .from('speakers')
    .select('id, name, email, bio, company, role, mini_bio, linkedin_url, pronouns, created_at')
  ...
  const speakers = (speakerData ?? []) as unknown as Array<{
    ...
    linkedin_url: string | null; pronouns: string | null; created_at: string
  }>
  ...
  const { data: invitationData } = await supabase
    .from('invitations')
    .select('id, email, status, created_at, expires_at, speaker_id, token')
    .eq('role', 'speaker')
    .eq('event_id', event.id)
    .in('status', ['pending'])
  ...
  const pendingInvitations = (invitationData ?? []) as unknown as Array<{
    id: string; email: string; status: string; created_at: string; expires_at: string; speaker_id: string | null; token: string
  }>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6 md:mb-8">
        <div>
          ...
          <p className="text-[13px] text-text3 mt-1">
            Gerencie palestrantes, convide por e-mail ou importe uma lista CSV
          </p>
        </div>
