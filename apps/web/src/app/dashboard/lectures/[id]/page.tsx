...
export default async function LectureDetailPage({ params }: Props) {
  ...
  // Fetch lecture + all speakers for editing
  const [{ data }, { data: allSpeakers }] = await Promise.all([
    supabase
      .from('lectures')
  ...
  const lecture = data as unknown as {
    ...
    card_images: string[] | null; processing_progress: number; event_id: string;
  ...
  return (
    <div className="max-w-5xl">
      ...
      <LectureDetailClient
        ...
        speakers={(allSpeakers as { id: string; name: string }[]) ?? []}
      />
    </div>
  )
}
