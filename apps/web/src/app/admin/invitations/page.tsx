...
export default function InvitationsPage() {
  ...
  async function handleInvite(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/send-invitation`, {
        ...
        body: JSON.stringify({ email, role: inviteRole, origin: window.location.origin }),
      })
  ...
  async function revokeInvitation(id: string) {
    ...
    await (supabase.from('invitations') as any).update({ status: 'revoked' }).eq('id', id)
    loadInvitations()
  }
  ...
  async function resendInvitation(inv: Invitation) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase.from('invitations') as any).delete().eq('id', inv.id)
    ...
    try {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/send-invitation`, {
        ...
        body: JSON.stringify({ email: inv.email, role: inv.role, origin: window.location.origin }),
      })
