...
function SetPasswordContent() {
  ...
  useEffect(() => {
    // Recovery flow: Supabase redirects here with #access_token=...&type=recovery
    // The browser client auto-detects these tokens and establishes a session.
    ...
    async function initSession() {
      ...
      if (hash && (hash.includes('error=') || hash.includes('error_code='))) {
        ...
        console.warn(`Auth error in URL: ${errorDesc}`)
        setSessionError(true)
        return
      }

      // If hash contains tokens, clear any stale session first
      ...
      <div className="flex min-h-screen items-center justify-center bg-bg">
        <div className="w-full max-w-md bg-bg2 border border-border-subtle rounded-2xl p-8 text-center">
          ...
          <p className="text-[13px] text-text3 mt-3">
            O link expirou. Solicite um novo convite ao organizador do evento.
          </p>
          <div className="flex flex-col sm:flex-row gap-2 mt-6 justify-center">
            <button
              onClick={() => router.push('/auth/forgot-password')}
              className="px-6 py-2.5 rounded-lg bg-purple text-white text-[13px] font-medium hover:bg-purple-light transition-all"
            >
              Redefinir senha
            </button>
            <button
