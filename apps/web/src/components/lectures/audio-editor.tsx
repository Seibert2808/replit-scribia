...
import { Upload, Scissors, AlertTriangle, Loader2, Mic, Trash2, X } from 'lucide-react'
...
const ACCEPTED_MIME_TYPES = ['audio/mpeg', 'audio/wav', 'audio/x-wav', 'audio/webm', 'audio/mp4', 'audio/x-m4a', 'audio/ogg', 'audio/vorbis']
const ACCEPTED_EXTENSIONS = ['.mp3', '.wav', '.webm', '.m4a', '.ogg']
const MAX_SIZE_MB = 500

function isValidAudioFile(file: File): boolean {
  const ext = '.' + file.name.split('.').pop()?.toLowerCase()
  const hasValidExtension = ACCEPTED_EXTENSIONS.includes(ext)
  const hasValidMime = ACCEPTED_MIME_TYPES.some(mime => file.type.startsWith(mime)) || file.type === ''
...
export function AudioEditor({ lectureId, eventId, hasAudio, onAudioDeleted }: AudioEditorProps) {
  ...
  async function handleFileSelect(file: File) {
    ...
    if (!isValidAudioFile(file)) {
      ...
      setError(`O arquivo "${file.name}" está no formato ${ext}, que não é suportado para transcrição. Envie o áudio em MP3, WAV, WebM, M4A ou OGG.`)
      return
    }

    // Validate size
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      setError(`O arquivo é muito grande (${Math.round(file.size / 1024 / 1024)}MB). O tamanho máximo permitido é ${MAX_SIZE_MB}MB.`)
      return
    }
  ...
  return (
    <div className="bg-bg2 border border-border-subtle rounded-[14px] overflow-hidden animate-fade-up">
      ...
      <div className="p-5">
        ...
        {error && (
          <div className="bg-scribia-red/8 border border-scribia-red/20 rounded-xl px-4 py-3 mt-3 animate-fade-up">
            <div className="flex items-start gap-3">
              <AlertTriangle className="w-4 h-4 text-scribia-red shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
                <p className="text-[12px] text-scribia-red font-medium mb-0.5">Erro no upload</p>
                <p className="text-[11px] text-text2 leading-relaxed">{error}</p>
              </div>
              <button
                onClick={() => setError(null)}
                className="p-0.5 rounded-md text-text3 hover:text-text hover:bg-bg3 transition-colors shrink-0"
              >
                <X className="w-3 h-3" />
              </button>
            </div>
