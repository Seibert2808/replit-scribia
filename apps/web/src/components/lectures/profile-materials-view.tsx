...
export function ProfileMaterialsView({ lectureId, selectedProfiles }: Props) {
  ...
  async function deleteProfile(profile: string) {
    if (!confirm(`Excluir playbook e live book do perfil "${PROFILE_LABELS[profile]?.label || profile}"?`)) return
    setDeleting(profile)
    try {
            ...
            >
              ...
              {isExpanded && (
                <div className="border-t border-border-subtle p-3 space-y-2">
                  {[
                    { type: 'ebook' as const, material: ebook, icon: BookOpen, label: 'Playbook' },
                    { type: 'playbook' as const, material: playbook, icon: FileText, label: 'Live Book' },
                  ...
                  {profileMaterials.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-border-subtle">
                      ...
                      >
                        ...
                        Excluir playbook e live book deste perfil
                      </button>
                    </div>
                  )}
                </div>
