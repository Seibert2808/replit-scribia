...
const typeLabels: Record<string, { label: string; className: string }> = {
  slides: { label: 'Slides', className: 'bg-primary/10 text-primary' },
...
export function FileList({ files, canDelete, onFileDeleted }: FileListProps) {
  ...
  async function handleDelete(file: SpeakerFile) {
    ...
    if (storagePath) {
      await supabase.storage.from('speaker-uploads').remove([storagePath])
    }
    ...
    return (
      <div className="text-center py-10 bg-bg3 rounded-lg">
        ...
        <p className="text-[12px] text-text3 mt-0.5">
          Envie slides, bibliografias ou materiais complementares.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-2">
      {files.map((file) => {
        ...
        return (
          <div
            key={file.id}
            className="flex items-center justify-between bg-bg3 rounded-lg p-3"
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={cn('px-1.5 py-0.5 rounded text-[10px] font-medium', typeCfg.className)}>
                    {typeCfg.label}
                  </span>
                  {file.file_size_bytes && (
                    <span className="text-[10px] text-text3">{formatSize(file.file_size_bytes)}</span>
                  )}
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <a
                ...
                className="p-1.5 text-text3 hover:text-primary transition-colors"
                title="Download"
              >
              ...
              {canDelete && (
                <button
                  ...
                  className="p-1.5 text-text3 hover:text-red-500 transition-colors disabled:opacity-50"
                  title="Excluir"
                >
