...
const LEVELS: {
...
}[] = [
  {
    id: 'junior',
    label: 'Iniciante',
    subtitle: 'Estudante ou início de carreira',
    description: 'Conteúdo com explicações detalhadas, glossário de termos e linguagem acessível.',
    icon: GraduationCap,
  ...
  },
  {
    id: 'pleno',
    label: 'Intermediário',
    subtitle: '2-5 anos de experiência',
    description: 'Conteúdo com insights práticos, frameworks e comparativos para aplicação imediata.',
    icon: Briefcase,
  ...
  },
  {
    id: 'senior',
    label: 'Avançado',
    subtitle: 'Especialista com 5+ anos',
    description: 'Conteúdo denso com análise estratégica, nuances e posicionamento bibliográfico.',
    icon: Award,
...
export function SenioritySelector({ lectureId, userId, onSelect }: SenioritySelectorProps) {
  ...
  async function handleSelect(level: SeniorityLevel) {
    ...
    try {
      await supabase
        .from('lecture_access')
        .update({ seniority_level: level } as never)
    ...
    <div className="animate-fade-up">
      <div className="text-center mb-6">
        ...
        <p className="text-[13px] text-text3 mt-1">
          Vamos personalizar o conteúdo para o seu perfil. Você pode trocar depois.
        </p>
      </div>
        ...
        {LEVELS.map((level) => {
          ...
          return (
            <button
              ...
              disabled={isDisabled}
              className="group relative flex flex-col items-center text-center bg-bg2 border border-border-subtle rounded-xl p-5 hover:border-border-purple hover:bg-bg3/50 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-wait"
            >
              <div className={`w-12 h-12 rounded-full bg-bg3 border border-border-subtle flex items-center justify-center mb-3 group-hover:border-border-purple transition-colors ${isLoading ? 'animate-pulse' : ''}`}>
                {isLoading ? (
                  <Loader2 className="w-5 h-5 text-purple-light animate-spin" />
                ) : (
