...
impl SharedCaptureData {
    ...
    pub fn get_status(&self) -> CaptureStatus {
        ...
        let chunk_count = *self.chunk_count.lock().unwrap();

        CaptureStatus {
            state,
            duration_seconds,
            audio_level,
            chunks_saved: chunk_count + 1, // chunk_count is 0-indexed
            output_dir,
        }
    }
    ...
    pub fn resume(&self) -> Result<(), String> {
        ...
        // Create a new chunk file for the resumed segment
        let mut chunk_count = self.chunk_count.lock().unwrap();
        *chunk_count += 1;
        let output_dir = self.output_dir.lock().unwrap().clone();
        ...
        let chunk_path = std::path::PathBuf::from(&output_dir)
            .join(format!("chunk_{}.wav", *chunk_count));
        let writer = WavWriter::create(chunk_path, spec)
            .map_err(|e| format!("WAV create on resume: {}", e))?;
        *self.writer.lock().unwrap() = Some(writer);
        // Reset resample position for the new chunk
        *self.resample_pos.lock().unwrap() = 0.0;
