...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (hash && (hash.includes('access_token') || hash.includes('type=invite') || hash.includes('type=magiclink') || hash.includes('type=recovery'))) {
      navigate(`/auth/set-password${window.location.search}${hash}`)
    }
  }, [navigate])
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-bg">
        <div className="w-full max-w-md bg-bg2 border border-border-subtle rounded-2xl p-8 animate-fade-up">
          ...
          <h1 className="font-heading text-2xl font-bold text-text text-center">Verifique seu email</h1>
          <p className="text-[13px] text-text2 text-center mt-3">
            Enviamos um link mágico para <strong className="text-purple-light">{email}</strong>.<br />Clique no link para entrar.
          </p>
        </div>
      </div>
    )
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg">
      <div className="w-full max-w-md bg-bg2 border border-border-subtle rounded-2xl p-8 animate-fade-up">
        <div className="text-center mb-8">
          ...
          <p className="text-[13px] text-text3 mt-2">Acesse sua conta para continuar</p>
        </div>

        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label htmlFor="email" className="block text-[12px] font-medium text-text2 mb-1.5">Email</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="password" className="block text-[12px] font-medium text-text2">Senha</label>
              <Link href="/auth/forgot-password" className="text-[12px] text-purple-light hover:text-purple transition-colors">Esqueceu a senha?</Link>
            </div>
            <div className="relative">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center"><span className="bg-bg2 px-3 text-[11px] uppercase text-text3 tracking-wider">ou</span></div>
        </div>

        <button onClick={handleMagicLink} disabled={!email || loading} className="w-full py-3 rounded-lg bg-transparent border border-border-subtle text-[14px] text-text2 hover:border-border-purple hover:text-purple-light disabled:opacity-50 transition-all">
          Entrar com link mágico
        </button>

        <p className="text-center text-[13px] text-text3 mt-6">
          Não tem conta?{' '}
          <Link href="/register" className="text-purple-light hover:text-purple transition-colors">Criar conta</Link>
        </p>
      </div>
    </div>
  )
