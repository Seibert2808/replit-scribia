...
export function UserMenu({ userName, userRole, variant = 'sidebar' }: UserMenuProps) {
  ...
  if (variant === 'navbar') {
    return (
      <div className="relative" ref={ref}>
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2.5 cursor-pointer rounded-lg px-2 py-1.5 transition-colors hover:bg-bg3">
          <div className="w-8 h-8 rounded-full bg-purple-dim border border-border-purple flex items-center justify-center text-[11px] font-heading font-bold text-purple-light">{initials}</div>
          <span className="text-[13px] text-text2">{userName}</span>
          <ChevronUp className={`w-3.5 h-3.5 text-text3 transition-transform duration-200 ${open ? '' : 'rotate-180'}`} />
        </button>
        {open && (
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center gap-2.5 px-3 py-3 cursor-pointer rounded-lg transition-colors hover:bg-bg3">
        <div className="w-8 h-8 rounded-full bg-purple-dim border border-border-purple flex items-center justify-center text-xs font-heading font-bold text-purple-light shrink-0">{initials}</div>
        <div className="flex-1 min-w-0 text-left">
          <div className="text-[12.5px] font-medium text-text truncate">{userName}</div>
          <div className="text-[10px] text-text3">{userRole}</div>
        </div>
        <ChevronUp className={`w-3.5 h-3.5 text-text3 shrink-0 transition-transform duration-200 ${open ? '' : 'rotate-180'}`} />
      </button>
      {open && (
