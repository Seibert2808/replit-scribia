...
export function ProgressBar({ label, value, max, color = 'purple', className }: ProgressBarProps) {
  const pct = max > 0 ? Math.round((value / max) * 100) : 0
  return (
    <div className={cn('mb-4 last:mb-0', className)}>
      <div className="flex justify-between mb-1.5">
        <span className="text-[12.5px] text-text2">{label}</span>
        <span className="text-xs text-text3">{value}/{max}</span>
      </div>
      <div className="h-1 bg-bg4 rounded-sm overflow-hidden">
        <div className={cn('h-full rounded-sm transition-all duration-500', colorClass[color])} style={{ width: `${pct}%` }} />
      </div>
    </div>
  )
}
