...
export default function LoginPage() {
  // If login page receives an access_token hash (invite/magic link redirect),
  // forward to set-password page which handles session establishment
  useEffect(() => {
    ...
    if (hash && (hash.includes('access_token') || hash.includes('type=invite') || hash.includes('type=magiclink') || hash.includes('type=recovery'))) {
      window.location.href = `/auth/set-password${window.location.search}${hash}`
    }
  }, [])
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="password" className="block text-[12px] font-medium text-text2">
                Senha
              </label>
              <Link
