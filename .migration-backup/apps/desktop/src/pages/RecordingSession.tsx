        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="animate-fade-up">
          ...
          {status === 'stopped' && deployStatus === 'none' && (
            <div className="card" style={{
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12, alignItems: 'center' }}>
                ...
                <button
                  onClick={async () => {
                    if (!confirm('Descartar o áudio local e iniciar nova gravação?')) return
                    try {
                      if (outputDir) {
                        await invoke('clean_local_chunks', { outputDir })
                      }
                      setStatus('idle')
                      setDuration(0)
                      setOutputDir('')
                      addLog('Áudio local descartado — pronto para nova gravação', 'ok')
                    } catch (e) {
                      addLog(`Erro ao limpar: ${e}`, 'error')
                    }
                  }}
          ...
          {status === 'stopped' && deployStatus === 'published' && (
            <div className="card" style={{ padding: 20, background: 'var(--success-dim)', border: '1px solid var(--success-border)', borderRadius: 10 }}>
              ...
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--success-border)' }}>
                <button
                  onClick={async () => {
                    if (!confirm('Deseja apagar o áudio atual e gravar novamente? Esta ação não pode ser desfeita.')) return
                    try {
                      // Delete remote audio files from Supabase storage
                      const storagePath = `${eventId}/${lectureId}`
                      ...
                      await supabase.from('lectures').update({
                        ...
                        summary: null,
                        topics: null,
                      } as never).eq('id', lectureId)
                      // Delete processing jobs and materials
                      await supabase.from('processing_jobs').delete().eq('lecture_id', lectureId)
                      await supabase.from('lecture_materials').delete().eq('lecture_id', lectureId)
                      await supabase.from('generation_configs').delete().eq('lecture_id', lectureId)
                      // Clean local files too
                      if (outputDir) {
                        try { await invoke('clean_local_chunks', { outputDir }) } catch { /* ignore */ }
                      }
                      setStatus('idle')
                      setDeployStatus('none')
                      setDeployProgress(0)
                      setDuration(0)
                      addLog('Áudio anterior excluído — pronto para nova gravação', 'ok')
                    } catch (e) {
                      setError(`Erro ao excluir: ${e}`)
                      addLog(`Erro ao excluir áudio: ${e}`, 'error')
                    }
                  }}
                  className="btn btn-ghost btn-sm"
                  style={{ fontSize: 11, color: 'var(--red)' }}
          ...
          {(status === 'recording' || status === 'paused') && (
            <div className="upload-card">
              ...
              <div style={{ fontSize: 10.5, color: 'var(--text3)', marginTop: 6 }}>
                ...
                {status === 'paused' && ' · Novo segmento ao retomar'}
              </div>
            </div>
          )}
