...
export interface EventBranding {
  primaryColor?: string   // hex e.g. '#6B4EFF'
  secondaryColor?: string // hex e.g. '#00D4A0'
  logoBytes?: Uint8Array  // PNG/JPEG bytes of event logo
...
export async function generatePdfFromMarkdown(
  ...
  // Resolve branding colors (event overrides or ScribIA defaults)
  const accent = options.branding?.primaryColor ? hexToRgb(options.branding.primaryColor) : COLORS.purple
  const accentLight = options.branding?.primaryColor
    ? hexToRgb(options.branding.primaryColor) // used where purpleLight was
    : COLORS.purpleLight
  const accent2 = options.branding?.secondaryColor ? hexToRgb(options.branding.secondaryColor) : COLORS.green
  ...
  try {
    page.drawText(badgeText, {
      x: MARGIN + 12, y: PAGE_H - 48, size: 11, font: boldFont, color: accentLight,
    })
  ...
  try {
    page.drawText(safeRaw(options.subtitle), {
      x: MARGIN, y: subtitleY, size: 14, font, color: accentLight,
    })
  } catch { /* skip */ }
  ...
  // ─── Event logo + ScribIA branding at bottom of cover ───
  let brandingY = 80
  // Event logo (if provided)
  if (options.branding?.logoBytes) {
    try {
      const logoImage = options.branding.logoMimeType === 'image/jpeg'
        ? await doc.embedJpg(options.branding.logoBytes)
        : await doc.embedPng(options.branding.logoBytes)
      const logoMaxH = 40
      const logoMaxW = 120
      const scale = Math.min(logoMaxW / logoImage.width, logoMaxH / logoImage.height)
      const logoW = logoImage.width * scale
      const logoH = logoImage.height * scale
      page.drawImage(logoImage, { x: MARGIN, y: brandingY - logoH + 30, width: logoW, height: logoH })
      // ScribIA branding to the right of event logo
      page.drawRectangle({
        x: MARGIN + logoW + 15, y: brandingY + 10, width: 1, height: 20, color: COLORS.divider,
      })
      try {
        page.drawText('Gerado por', {
          x: MARGIN + logoW + 25, y: brandingY + 15, size: 8, font, color: COLORS.textMuted,
        })
        page.drawText('SCRIBIA', {
          x: MARGIN + logoW + 25, y: brandingY, size: 12, font: boldFont, color: accent,
        })
      } catch { /* skip */ }
    } catch (e) {
      console.warn('Failed to embed event logo:', e)
      // Fallback: just show ScribIA branding
      page.drawRectangle({
        x: MARGIN, y: brandingY, width: 120, height: 1, color: accent, opacity: 0.3,
      })
      try {
        page.drawText('Gerado por', { x: MARGIN, y: 55, size: 10, font, color: COLORS.textMuted })
        page.drawText('SCRIBIA', { x: MARGIN + 70, y: 55, size: 14, font: boldFont, color: accent })
      } catch { /* skip */ }
    }
  } else {
    // No event logo — ScribIA branding only
    page.drawRectangle({
      x: MARGIN, y: brandingY, width: 120, height: 1, color: accent, opacity: 0.3,
    })
    try {
  ...
  // Accent bar bottom of cover
  page.drawRectangle({
    x: 0, y: 0, width: PAGE_W, height: 4, color: accent,
  })
  ...
  if (chapters.length > 0) {
    ...
    y -= 10
    page.drawRectangle({ x: MARGIN, y, width: 50, height: 3, color: accent })
    y -= 25
    ...
    for (let i = 0; i < chapters.length; i++) {
      ...
      try {
        page.drawText(num, { x: MARGIN, y, size: 12, font: boldFont, color: accent })
  ...
  for (const section of sections) {
    switch (section.type) {
      ...
      case 'h2': {
        ...
        try {
          page.drawText(`CAPITULO ${chapterNum}`, {
            x: MARGIN, y, size: 10, font: boldFont, color: accent,
          })
        } catch { /* skip */ }
        y -= 8
        page.drawRectangle({ x: MARGIN, y, width: 40, height: 3, color: accent })
        y -= 22
      ...
      case 'h3': {
        ...
        page.drawCircle({ x: MARGIN + 4, y: y + 5, size: 3, color: accent })
      ...
      case 'bullet': {
        ...
        for (const item of section.items) {
          ...
          try {
            page.drawText('\u2022', { x: MARGIN + 8, y, size: 11, font, color: accent })
          } catch {
            page.drawText('-', { x: MARGIN + 8, y, size: 11, font, color: accent })
          }
      ...
      case 'numbered': {
        ...
        for (let i = 0; i < section.items.length; i++) {
          ...
          const numStr = `${i + 1}.`
          try {
            page.drawText(numStr, { x: MARGIN + 4, y, size: 11, font: boldFont, color: accent })
          } catch { /* skip */ }
      ...
      case 'checkbox': {
        ...
        for (let i = 0; i < section.items.length; i++) {
          ...
          page.drawRectangle({
            ...
            color: isChecked ? accent : COLORS.white,
          })
          if (isChecked) {
            // Simple checkmark
            try {
  ...
  for (let i = 0; i < totalPages; i++) {
    ...
    try {
      p.drawText('ScribIA', {
        x: MARGIN, y: 25, size: 9, font: boldFont, color: accent, opacity: 0.5,
      })
    } catch { /* skip */ }
    // Bottom accent line
    p.drawRectangle({
      x: 0, y: 18, width: PAGE_W, height: 0.5, color: accent, opacity: 0.15,
    })
  }
