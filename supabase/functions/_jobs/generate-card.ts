// Story 6.1 (evolved): Card generation — 3 social media cards per lecture
// Uses AI to extract 3 impactful phrases + generates SVG cards with different designs
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: job } = await supabase
      ...
      .select('*, lectures(id, event_id, title, summary, topics, speakers:speaker_id(name), events:event_id(name, primary_color, secondary_color, logo_url))')
      .eq('type', 'card')
      .eq('status', 'queued')
    ...
    // Use AI to extract 3 impactful phrases for the cards
...
- Responda APENAS com o JSON, sem texto adicional`
      ...
      const aiResult = await callAIProvider(aiPrompt, {
        temperature: 0.8,
        maxTokens: 512,
        jsonMode: true,
      })

      phrases = JSON.parse(aiResult.trim())
    } catch {
    ...
    // Generate 3 different card SVGs
    const W = 1080, H = 1080 // Instagram feed format

    const cards = [
      {
        name: 'card_quote',
        svg: generateQuoteCard(phrases.quote, speaker, eventName, primaryColor, secondaryColor, W, H),
      },
      {
        name: 'card_title',
        svg: generateTitleCard(phrases.title_phrase, speaker, eventName, title, primaryColor, secondaryColor, W, H),
      },
      {
        name: 'card_insight',
        svg: generateInsightCard(phrases.insight, speaker, eventName, topics, primaryColor, secondaryColor, W, H),
      },
    ]
    ...
    for (const card of cards) {
      ...
      await supabase.storage.from('materials').upload(path, new TextEncoder().encode(card.svg), {
        contentType: 'image/svg+xml', upsert: true,
      })
      cardPaths.push(path)
    }

    // Also keep backwards-compatible card_image_url (first card)
    ...
    return new Response(JSON.stringify({ success: true, cards: cardPaths }), { status: 200 })
  } catch (e) {
...
// Word-wrap text into lines of max width (approximate)
...
// ─── Card 1: Quote Card ─────────────────────────────────
// Large quote with speaker attribution, dramatic dark background
...
// ─── Card 2: Title Card ──────────────────────────────────
// Bold title with gradient background, modern layout
...
function generateTitleCard(
  ...
  <!-- Title -->
  ${titleText}
  <!-- Subtitle phrase if different from title -->
...
// ─── Card 3: Insight Card ────────────────────────────────
// Key insight with topic pills, data-driven feel
